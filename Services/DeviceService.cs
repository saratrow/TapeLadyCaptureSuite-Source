...
internal static class DeviceService
{
        ...
        CaptureDeviceInfo deviceInfo)
    {
        ...
        try
        {
            ...
            while (pinEnumerator.Next(1, pins, IntPtr.Zero) == 0)
            {
                ...
                try
                {
                    ...
                    var pinName = GetPinName(pin);
                    if (PinSupportsMajorType(pin, MediaType.Audio))
                    {
                        result.Add(string.IsNullOrWhiteSpace(pinName)
                            ? "Audio Output"
                            : pinName);
                    }
                }
        ...
        catch
        {
            ...
            // The normal Windows audio-device list remains available as fallback.
        }
        finally
        {
    ...
    private static string GetPinName(IPin pin)
    {
        ...
        try
        {
            return pinInfo.name?.Trim() ?? string.Empty;
        }
        finally
        {
    ...
    private static bool PinSupportsMajorType(IPin pin, Guid majorType)
    {
